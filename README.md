...
If we'd only tested models that already had math-specific training, we couldn't tell whether any improvement came from our distillation or from what the model already knew. Llama has no built-in reasoning mode like Qwen, and no documented math-RL training like Gemma — that makes it the cleanest test of the actual question: how much can generic distillation alone teach reasoning to a model that didn't start with any of that. It's the control case, not an odd choice.

Qwen is our upper-bound case, not just 'the biggest model.' It's the same family as our teacher, it has a built-in reasoning mode already aligned with the kind of step-by-step supervision we're providing, and it's the largest of the three students. We picked it specifically to show what distillation looks like under the most favorable conditions — same tokenizer, same training lineage, reasoning-oriented architecture already in place. Without that comparison point, we'd have no way to know whether 50% was close to the best any small model could do, or whether the ceiling was actually much higher. Qwen answers that: it closed to within 12.7 points of the teacher, confirming our prediction that it would reach the highest absolute accuracy

Gemma was picked to test a specific, different hypothesis — not architecture in general, but whether existing math specialization survives a second round of generic distillation. Gemma's own training already includes documented reinforcement learning on math problems, so we predicted it would start ahead of Llama and keep some of that edge after fine-tuning. That's exactly the kind of prediction you can only test by picking a model that already has domain-specific training going in — and this is actually where our project produced its most interesting result: Gemma's baseline prediction held, but the 'retain the edge' part didn't. It regressed instead, which tells us something real about how generic SFT can interact with — and potentially overwrite — RL-shaped behavior.
